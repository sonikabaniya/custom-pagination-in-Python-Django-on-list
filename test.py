...
from content.models import FirstTestModel, SecondTestModel
    ...
    def get(self, request):
        ...
        try:
            if insight_type in ["News", "Blog"] or insight_type is None:
                results_insight = FirstTestModel.objects.filter(**filter_args, **filter_insights_args).values(
            ...
            if insight_type in ["Resources"] or insight_type is None:
                results_resources = SecondTestModel.objects.filter(**filter_args).values(
